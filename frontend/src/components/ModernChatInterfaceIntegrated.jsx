...
const ModernChatInterfaceIntegrated = () => {
  const {
    selectedModel,
    isLoading,
    error,
  ...
  const [showModelSelector, setShowModelSelector] = useState(!selectedModel);
  ...
  const spiritualPrompts = [
    ...
    "Qual o significado da vida segundo sua tradição?",
    "Como superar o sofrimento emocional?",
    "Práticas para despertar a consciência"
  ];
  ...
  useEffect(() => {
    // Simular mudanças de humor da IA baseado no modelo selecionado
    if (selectedModel) {
  ...
  const getConnectionStatusIcon = () => {
    switch (connectionStatus) {
      case 'connected':
        return <RiCheckCircleLine className="text-green-400" />;
      case 'connecting':
        return <RiWifiLine className="text-yellow-400 animate-pulse" />;
      case 'error':
        return <RiAlertCircleLine className="text-red-400" />;
      default:
        return <RiWifiOffLine className="text-gray-400" />;
    }
  };
  ...
  const MessageBubble = ({ message, index }) => {
    ...
    const personality = aiPersonalities[aiMood] || aiPersonalities.peaceful;
      ...
      >
        <div className={`flex items-end gap-3 max-w-[80%] ${isAI ? 'flex-row' : 'flex-row-reverse'}`}>
          ...
          >
            {isAI ? (
              selectedModel?.icon ? (
                <selectedModel.icon size={20} style={{ color: selectedModel.color }} />
              ) : (
          ...
          >
            ...
            {message.error && (
              ...
              >
                <RiAlertCircleLine size={12} />
                <span>Resposta com erro técnico</span>
              </motion.div>
            )}
              ...
              {isAI && (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs" style={{ color: personality.color }}>
                    {message.model || personality.name}
                  </span>
                </div>
              )}
            </motion.div>
  ...
  // Mostrar seletor de modelo se não há modelo selecionado
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              >
                {selectedModel?.icon ? (
                  <selectedModel.icon size={24} style={{ color: selectedModel.color }} />
                ) : (
              ...
              <div>
                <h2 className="text-xl font-semibold text-white">
                  {selectedModel?.name || 'Guia Espiritual IA'}
                </h2>
                <motion.div
        ...
        <AnimatePresence>
          {hasError && (
            <motion.div
              ...
              className="bg-red-500/20 backdrop-blur-lg border-b border-red-500/30 p-4"
            >
              <div className="flex items-center gap-3">
                <RiAlertCircleLine size={20} className="text-red-400" />
          ...
          <div className="flex gap-2 overflow-x-auto pb-2">
            {spiritualPrompts.map((prompt, index) => (
              <motion.button
                ...
                className="flex-shrink-0 px-3 py-2 bg-white/10 backdrop-blur-lg rounded-full text-sm text-white/80 border border-white/20 hover:border-purple-400/50 transition-all duration-300 disabled:opacity-50"
              >
                {prompt}
              </motion.button>
            ))}
          ...
          <div className="flex items-end gap-3">
            <div className="flex-1 relative">
              <motion.textarea
                ...
                placeholder={isConnected ? "Digite sua pergunta espiritual..." : "Conecte-se a um modelo primeiro..."}
                disabled={!isConnected || isTyping}
                className="w-full bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl px-4 py-3 pr-12 text-white placeholder-white/50 resize-none focus:outline-none focus:border-purple-400/50 transition-all duration-300 disabled:opacity-50"
                rows={1}
            ...
            <motion.button
              ...
              className={`p-3 rounded-full transition-all duration-300 ${
                inputMessage.trim() && isConnected && !isTyping
...
export default ModernChatInterfaceIntegrated;
